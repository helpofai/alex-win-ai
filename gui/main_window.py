...
from gui.click_ripple import ClickRipple
...
class MiniSphere(QWidget):
    def __init__(self, parent=None):
        ...
        self.resize(100, 100)
        self.layout = QVBoxLayout(self); self.reactor = ReactorWidget(); self.reactor.setFixedSize(80, 80); self.layout.addWidget(self.reactor)
        self.mouse_pos = None
    def set_state(self, state): self.reactor.set_state(state)
    ...
    def mouseDoubleClickEvent(self, e): self.parent_window.restore_from_mini()

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"ALEX v{CURRENT_VERSION} | COMMAND SPHERE")
        ...
        self.signals.show_ripple.connect(self.ripple.trigger)

        self.mini_mode = MiniSphere(); self.mini_mode.parent_window = self
        self.main_container = QWidget(); self.main_container.setObjectName("CentralWidget")
        self.setCentralWidget(self.main_container); self.main_layout = QVBoxLayout(self.main_container); self.main_layout.setContentsMargins(30, 30, 30, 30)

        header = QHBoxLayout()
        self.logo_label = QLabel(f"ALEX AGENTIC CORE // v{CURRENT_VERSION}"); self.logo_label.setObjectName("HeaderLabel"); header.addWidget(self.logo_label); header.addStretch()
        self.time_label = QLabel("SYSTEM TIME: 00:00:00"); header.addWidget(self.time_label)
        self.mini_btn = QPushButton("MINI"); self.mini_btn.setFixedSize(60, 30); self.mini_btn.clicked.connect(self.switch_to_mini); header.addWidget(self.mini_btn)
        close_btn = QPushButton("✕"); close_btn.setFixedSize(40, 40); close_btn.clicked.connect(self.close); header.addWidget(close_btn)
        ...
        self.audio_bar = AudioBar(); center_core.addStretch(); center_core.addWidget(self.reactor, 0, Qt.AlignCenter); center_core.addWidget(self.audio_bar, 0, Qt.AlignCenter); center_core.addStretch(); sphere_area.addLayout(center_core, stretch=2)

        self.right_wing = QFrame(); self.right_wing.setObjectName("WingPanel"); self.right_wing.setFixedWidth(300)
        right_layout = QVBoxLayout(self.right_wing); right_layout.addWidget(QLabel("COMMUNICATIONS")); self.chat_area = QTextEdit(); self.chat_area.setReadOnly(True); right_layout.addWidget(self.chat_area); sphere_area.addWidget(self.right_wing)
        self.main_layout.addLayout(sphere_area)

        input_container = QHBoxLayout(); self.input_field = QLineEdit(); self.input_field.setPlaceholderText("TRANSMIT COMMAND..."); self.input_field.returnPressed.connect(self.handle_text_input); input_container.addStretch(); input_container.addWidget(self.input_field, stretch=4); input_container.addStretch(); self.main_layout.addLayout(input_container)
